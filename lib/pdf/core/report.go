...
import (
	"github.com/zituocn/gow/lib/pdf/util"
	"io/ioutil"
	"os"
	"regexp"
	"strconv"
...
func CreateReport() *Report {
	...
	report.flags[FlagAutoAddNewPage] = false
	report.flags[FlagResetPageNo] = false
...
func (report *Report) executeDetail() {
	...
	if h != nil {
		if report.flags[FlagAutoAddNewPage] {
			report.AddNewPage(report.flags[FlagResetPageNo])
			report.flags[FlagAutoAddNewPage] = false
			report.flags[FlagResetPageNo] = false
		}
...
func (report *Report) SetPage(size string, orientation string) {
	...
	case "A5":
		switch orientation {
		case "P":
			report.addAtomicCell("P|" + unit + "|A5|P")
			report.pageWidth = config.width
			report.pageHeight = config.height
		case "L":
			report.addAtomicCell("P|" + unit + "|A5|L")
