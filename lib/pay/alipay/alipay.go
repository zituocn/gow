...
import (
	...
	"encoding/base64"
	"encoding/json"
	"github.com/zituocn/gow/lib/pay/alipay/encoding"
	"io"
	"io/ioutil"
	"net/http"
	"net/url"
...
//NewAliPay init a new client
func NewAliPay(appID, aliPublicKey, privateKey string, isProduction bool) (client *AliPay) {
	client = new(AliPay)
	client.appID = appID
...
//URLValues URLValues
//param to url
func (m *AliPay) URLValues(param AliPayParam) (url.Values, error) {
	var p = url.Values{}
	p.Add("app_id", m.appID)
	p.Add("method", param.APIName())
	p.Add("format", K_FORMAT)
	p.Add("charset", K_CHARSET)
	p.Add("sign_type", m.SignType)
	...
	if m.SignType == K_SIGN_TYPE_RSA {
		hash = crypto.SHA1
	} else {
		hash = crypto.SHA256
	}
	sign, err := signWithPKCS1v15(p, m.privateKey, hash)
	if err != nil {
		return nil, err
	}
	p.Add("sign", sign)
...
//doRequest doRequest
func (m *AliPay) doRequest(method string, param AliPayParam, results interface{}) (err error) {
	var buf io.Reader
	if param != nil {
	...
	req, err := http.NewRequest(method, m.apiDomain, buf)
	if err != nil {
		return err
	}
	...
	if len(m.AliPayPublicKey) > 0 {
		...
		if sign != "" {
			if ok, err := verifyData([]byte(content), m.SignType, sign, m.AliPayPublicKey); ok == false {
				return err
			}
		}
	}
...
//DoRequest DoRequest
func (m *AliPay) DoRequest(method string, param AliPayParam, results interface{}) (err error) {
	return m.doRequest(method, param, results)
}

//VerifySign 验证签名
func (m *AliPay) VerifySign(data url.Values) (ok bool, err error) {
	return verifySign(data, m.AliPayPublicKey)
}
